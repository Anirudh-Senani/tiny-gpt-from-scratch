...
def backward_through_all_blocks(d_y, caches, blocks):
    ...
    for i in range(len(blocks)-1, -1, -1):
        d_y, grads = transformer_block_backward(d_y, caches[i], blocks[i])
        grads_list[i] = grads
...
def full_model_forward(x_ids, model_params):
    ...
    emb, emb_cache = token_embedding_forward(x_ids, model_params['tok_emb'])
    pos_emb = slice_positional_embedding(model_params['pos_emb'], x_ids.shape[1])

    x = add_token_and_positional_embeddings(emb, pos_emb)
    y, blocks = forward_through_all_blocks(x, model_params['blocks'])
    y, ln_f = final_layernorm_forward(y, **model_params['ln_f'])
    lm_head = lm_head_linear_forward(y, **model_params['lm_head'])
    cache = dict(emb=emb_cache,blocks=blocks,ln_f=ln_f,lm_head=lm_head['cache'])
...
def wire_full_training_loop(params, train_ids, val_ids, block_size, batch_size, n_steps, lr, betas, eps):
    ...
    for step in range(n_steps):
        x_ids, y = get_batch(train_ids, block_size, batch_size, rng)
...
def logging_and_validation_loss(params, val_ids, block_size, batch_size, n_eval_batches):
    ...
    for _ in range(n_eval_batches):
        x_ids, y = get_batch(val_ids, block_size, batch_size, rng)
        logits, caches = full_model_forward(x_ids, params)
        exp_logits = np.exp(logits - logits.max(axis=-1, keepdims=True))
        softmax_logits = exp_logits/exp_logits.sum(axis=-1, keepdims=True)
        y_onehot = np.zeros_like(softmax_logits)
    ...
    return avg_loss/n_eval_batches
...
def forward_to_get_logits(params, context_ids):
    ...
    return full_model_forward(context_ids, params)[0]
